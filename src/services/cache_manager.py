...
def read_ohlcv_from_cache(cache_dir: str, symbol: str, timeframe: str = '4h') -> Optional[pd.DataFrame]:
    ...
    filepath = get_cache_filepath(cache_dir, symbol, timeframe)

    if not os.path.exists(filepath):
        # print(f"Cache file not found for {symbol} ({timeframe}) at {filepath}") # Debug
        return None
    try:
        ...
        if df.empty:
            # print(f"Cache file for {symbol} ({timeframe}) is empty.") # Debug
            return None

        # Ensure correct columns and sort by timestamp
        if not all(col in df.columns for col in CACHE_COLUMNS):
            # print(f"Cache file for {symbol} ({timeframe}) has incorrect columns.") # Debug
        ...
        # print(f"Successfully read cache for {symbol} ({timeframe}): {len(df)} records.") # Debug
        return df
    except pd.errors.EmptyDataError:
        # print(f"Cache file for {symbol} ({timeframe}) is empty or corrupted (EmptyDataError).") # Debug
        os.remove(filepath) # Remove empty/corrupt file
        return None
    except Exception as e:
        # print(f"Error reading cache file for {symbol} ({timeframe}) at {filepath}: {e}") # Debug
...
def write_ohlcv_to_cache(cache_dir: str, symbol: str, data: pd.DataFrame, timeframe: str = '4h'):
    ...
    if not all(col in data.columns for col in CACHE_COLUMNS):
        raise ValueError(f"DataFrame to be cached for {symbol} ({timeframe}) is missing required columns. Expected: {CACHE_COLUMNS}")
    ...
    try:
        ...
        # print(f"Successfully wrote {len(data_sorted)} records to cache for {symbol} ({timeframe}) at {filepath}") # Debug
    except Exception as e:
